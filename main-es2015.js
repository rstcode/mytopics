...
let AppComponent = class AppComponent {
    constructor(auth, sharedsvc, location) {
        this.auth = auth;
    ...
    ngOnInit() {
        ...
        this.sharedsvc.slider.subscribe((p) => { this.sliderFlag = p; });
...
let ErrorConnectionComponent = class ErrorConnectionComponent {
    constructor() { }
    ngOnInit() {
...
let HeaderComponent = class HeaderComponent {
    constructor(route, router, auth, sharedsvc) {
        this.route = route;
        this.router = router;
        this.auth = auth;
        this.sharedsvc = sharedsvc;
...
let HomeComponent = class HomeComponent {
    ...
    constructor(firebaseService, route, router, auth, sharedsvc) {
    ...
    loadTopicTypes() {
        this.firebaseService.getTopicTypes(this.uid).subscribe(data => {
            this.topicTypes = [];
    ...
    addTopicType() {
        ...
        const topicTypeObj = new _models_entities__WEBPACK_IMPORTED_MODULE_2__["TopicType"]();
    ...
    addTopicTypePopup() {
        const topicType = new _models_entities__WEBPACK_IMPORTED_MODULE_2__["TopicType"]();
        topicType.Type = '';
        topicType.UId = this.uid;
        this.popupData.formEntity = this.getFormEntityForTopicType(topicType, 'Add');
        this.popupData.formEntity.submitCallBack = (formEntity) => {
            topicType.Type = formEntity.formControls[0].val;
            console.log('submit add topic type event');
            this.firebaseService.addTopicType(topicType);
        };
        this.popupData.popupDisplay = 'block';
    }
    getFormEntityForTopicType(topicType, action) {
        const formEntity = new _models_entities__WEBPACK_IMPORTED_MODULE_2__["FormEntity"]();
    ...
    getFormControlsForTopicType(data) {
        return [
...
let LoginComponent = class LoginComponent {
    constructor(auth, route, router, loader) {
        this.auth = auth;
        this.route = route;
        this.router = router;
        this.loader = loader;
...
let PopupModelComponent = class PopupModelComponent {
    constructor() {
...
let SidebarComponent = class SidebarComponent {
    constructor(router, firebaseService, auth, sharedsvc, toastr) {
        this.router = router;
        this.firebaseService = firebaseService;
        this.auth = auth;
        this.sharedsvc = sharedsvc;
    ...
    addTopicType() {
        if (!this.auth.isAuthenticated()) {
            ...
            const topicTypeObj = new _models_entities__WEBPACK_IMPORTED_MODULE_3__["TopicType"]();
            topicTypeObj.Type = newTopicType;
            topicTypeObj.UId = this.uid;
            this.firebaseService.addTopicType(topicTypeObj);
...
let TopicComponent = class TopicComponent {
    // tslint:disable-next-line:max-line-length
    constructor(route, router, datepipe, firebaseService, auth, toastr, sharedsvc, loader) {
        this.route = route;
        this.router = router;
    ...
    GetTopicTypeKey() {
        this.loader.show();
        this.firebaseService.getTopicTypeKey(this.auth.getCurrentUserId()).subscribe(p => {
            // tslint:disable-next-line:prefer-const
            if (p != null && p.length > 0) {
                p.forEach(item => {
                    ...
                    // tslint:disable-next-line:no-string-literal
                    a['$key'] = item.key;
                    if (a.Type === this.currentTT) {
                        this.currentTTKey = item.key;
                    }
                });
                this.LoadTopics();
    ...
    LoadTopics() {
        this.firebaseService.getTopicsByType(this.currentTTKey).subscribe(data => {
    ...
    AddTopic() {
        this.loader.show();
        const topic = new _models_entities__WEBPACK_IMPORTED_MODULE_5__["Topic"]();
        topic.Header = '';
        topic.Description = '';
        ...
        this.popupData.formEntity.submitCallBack = (formEntity) => {
            ...
            this.firebaseService.addTopic(topic);
    ...
    EditTopic(topic) {
        ...
        this.popupData.formEntity.submitCallBack = (formEntity) => {
            ...
            this.firebaseService.updateTopic(topic).then(p => {
            });
        };
        this.popupData.popupDisplay = 'block';
    }
    ...
    getFormEntityForTopic(topic, title) {
        const formEntity = new _models_entities__WEBPACK_IMPORTED_MODULE_5__["FormEntity"]();
        formEntity.title = title + ' topic';
        formEntity.isSubmitBtn = true;
        formEntity.formControls = this.getFormControlsForTopic(topic);
        return formEntity;
    }
    getFormControlsForTopic(data) {
        return [
...
let DescMinPipe = class DescMinPipe {
...
let Str2objPipe = class Str2objPipe {
    transform(value, args) {
...
let FireAuthService = class FireAuthService {
    constructor(afAuth, afs, router, loader) {
        this.afAuth = afAuth;
        this.afs = afs;
        this.router = router;
        this.loader = loader;
...
let FireBaseService = class FireBaseService {
    ...
    getTopicsByType(key) {
        this.topicsRef = this.db.list('/posts', ref => ref.orderByChild('TopicType').equalTo(key));
